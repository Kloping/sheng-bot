...
```
QQ 用户 → Mirai/Overflow → OkHttp → Spring WebFlux → 业务逻辑
                                                        ↓
                                                  Spring AI → LLM
                                                        ↓
业务逻辑 ← JSON 处理 ← 响应生成 ← 记忆检索 ← Milvus/Redis/MySQL
    ↓
Mirai/Overflow → OkHttp → QQ 用户
```
...
- **主类**: top.kloping.code.gdpl2112.SuperOneBotApplication
- **生命周期阶段**: package
- **目标**: single
